<script lang="ts">
	...
	async function askGora(e: any) {
		try {
			...
			const fRes = await (await fetch(`/api/ask-gora?prompt={${message}}`)).json().catch((err) => {
				console.log(err);
				return {
...
<div class="">
	<button
		class="btn absolute bottom-0 left-0 mb-2 flex items-center justify-center rounded-full"
		onclick={() => {
			showChat = true;
		}}
	>
		Ask Gora <img src="/favicon.ico" width="20" alt="Gora icon" class="" />
	</button>
</div>
...
{#if showChat}
	...
	>
		...
		{#if !chatMessages.length}
			<div class="mx-10 mb-20 flex flex-col items-center justify-center">
				<p class=" text-2xl">
					Hello, <span
		...
		<div class="max-h-[45vh] overflow-auto" bind:this={chatDiv}>
			{#each chatMessages as message}
				<div class={`${message.name == 'Me' ? 'chat-end' : 'chat-start'} chat`}>
...
<!-- <div class="fab left-0 top-0">
	<div tabindex="0" role="button" class="btn btn-lg btn-ghost">
		 <img src="/favicon.ico" width="20" alt="Gora icon" class="" />
	</div>

	<div class="fab-main-action">
		Ask Gora <button class="btn btn-circle btn-ghost btn-lg"
			><img src="/favicon.ico" width="20" alt="Gora icon" class="" /></button
		>
	</div>

    <div class="bg-base-300 rounded-2xl p-3 h-[500px] w-[400px] relative">
